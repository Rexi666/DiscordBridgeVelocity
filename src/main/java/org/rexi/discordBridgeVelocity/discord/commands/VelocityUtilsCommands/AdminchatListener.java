...
public class AdminchatListener extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        if (!plugin.getConfig("velocity_utils_commands.enabled", false) || !plugin.getConfig("velocity_utils_commands.adminchat.enabled", true)) {
        ...
        if (message == null || message.isEmpty()) {
            event.reply(plugin.getConfig("discord_messages.admin_chat_usage", "Usage: `/adminchat <message>`"))
                    .setEphemeral(true).queue();
            return;
        }
